...
| 🎵 粤语歌跟唱 | 16 首经典曲目（Beyond、陳慧嫻、李克勤、陳奕迅、葉麗儀、羅文、鄧麗君、陳百強、林子祥）的逐句歌词、粤拼、释义与设备粤语朗读；支持录音跟唱、全文注音、曲目收藏、本地歌词练习器及导入本地音频；歌词为节选，本站不提供歌曲音源 |
...
浏览器 ── 前端应用（7 大模块，单页应用）
        ...
        ├── 浏览器本地存储 → 本机学习档案 / 私人歌词草稿 / 进度 / 收藏 / 打卡
        ├── /api/account → 受邀账户登录与学习档案同步（可选；私人歌词除外）
        ├── 私人歌词朗读 → 仅设备 Web Speech，不调用云端 TTS
        └── 导入本地音频 → 本机播放（不上传）
```

特点：无需注册即可学习，本机档案和 JSON 备份继续保留。登录受邀账户后，进度、目标、生词/曲目收藏、打卡和练习摘要会自动同步；私人歌词正文、歌词粤拼、录音、导入歌曲和密码不会进入云端档案。公开注册默认关闭。
...
- 曲目收藏只同步 `js/songs.js` 中的曲目 ID。用户粘贴的私人歌词保存在独立的本机草稿键中，不进入学习档案、JSON 档案导出或云端同步。
...
`js/songs.js` 中现有歌曲由 CI 锁定歌曲 ID、句数与内容指纹。修正粤拼、释义或发音提示前应先核对依据，并在人工复核后更新校验基线；不要新增未获授权的歌词、音频、封面或其他第三方内容。用户也可以在浏览器粘贴私人歌词或导入自己合法取得的本地音频。
...
- 数据驱动：学习内容在 `js/data.js`，歌曲逐句练习数据在 `js/songs.js`；歌词内容不在 MIT 授权范围内
- 朗读：公共课程内容走 `speak(text)`；全部歌词朗读走 `privatePracticeSpeak(text)`，只调用设备粤语，不得发送到云端 TTS。全局语速 `speechRate`（0.5~2 六档，localStorage 记忆），新增朗读调用不要写死 rate
...
- 歌曲及歌词版权归原作者和相关权利人所有，不随本仓库的 MIT License 授权；本站不提供歌曲音源。
- 逐句材料用于粤语发音学习；如权利人认为相关内容不宜展示，可联系维护者处理。
- 本地歌词练习器是用户侧工具，不代表网站取得所输入内容的版权；用户应只处理自己有权使用的文本。
